...
class CreatorMyEvents extends StatefulWidget {
  const CreatorMyEvents({super.key});

  @override
  _CreatorMyEventsState createState() => _CreatorMyEventsState();
}

class _CreatorMyEventsState extends State<CreatorMyEvents> {
  ...
  void _setupEventIdsStream() {
    ...
    if (currentUser != null) {
      _eventIdsStream = _firestore
          ...
          .snapshots()
          .map((snapshot) =>
              List<String>.from(snapshot.data()?['publishedEvents'] ?? []));
    }
  }
  ...
  Future<void> _fetchEventsData(List<String> eventIds) async {
    ...
    for (String id in eventIds) {
      var docSnapshot = await _firestore.collection('events').doc(id).get();
                        ...
                        return ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return EventCard(
                              ...
                              cardType: EventCardType.published,
