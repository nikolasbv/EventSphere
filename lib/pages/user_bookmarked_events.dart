...
class UserBookmarkedEvents extends StatefulWidget {
  const UserBookmarkedEvents({super.key});

  @override
  _UserBookmarkedEventsState createState() => _UserBookmarkedEventsState();
}

class _UserBookmarkedEventsState extends State<UserBookmarkedEvents> {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  List<DocumentSnapshot> _bookmarkedEvents = [];
  ...
  Future<void> _fetchBookmarkedEvents() async {
    List<DocumentSnapshot> bookmarkedEventsDocs = [];
    for (var eventId in _userBookmarkedEvents) {
      DocumentSnapshot eventSnapshot =
          await _firestore.collection('events').doc(eventId).get();
      if (eventSnapshot.exists) {
        bookmarkedEventsDocs.add(eventSnapshot);
      }
    }

    setState(() {
      _bookmarkedEvents = bookmarkedEventsDocs;
    });
  }
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  String eventId = _bookmarkedEvents[index].id;
  ...
  void _onSearchQueryChanged(String query) {
    setState(() {
      _searchQuery = query;
      _fetchFilteredBookmarkedEvents();
    });
  }
  ...
  Future<void> _fetchFilteredBookmarkedEvents() async {
    ...
    setState(() {
      _bookmarkedEvents = filteredEvents;
    });
  }
}
