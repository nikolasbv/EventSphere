...
class EventEditSavedPage extends StatefulWidget {
  final String eventId;
  final EventCardType cardType;

  const EventEditSavedPage(
      {super.key, required this.eventId, required this.cardType});

  @override
  _EventEditSavedPageState createState() => _EventEditSavedPageState();
}

class _EventEditSavedPageState extends State<EventEditSavedPage> {
  ...
  Future<void> fetchEventDetails(String eventId) async {
    ...
    try {
      DocumentSnapshot eventSnapshot =
          await _firestore.collection('savedEvents').doc(eventId).get();
  ...
  Future<void> _submitEvent() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        DocumentReference newEventRef = _firestore.collection('events').doc();
        ...
        if (_user != null) {
          ...
          await userRef.update({
            'publishedEvents': FieldValue.arrayUnion([newEventRef.id])
          });
        ...
        Navigator.pushAndRemoveUntil(
          context,
          MaterialPageRoute(
              builder: (context) => EventQRCodeScreen(eventId: newEventRef.id)),
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        DocumentReference eventRef =
            _firestore.collection('savedEvents').doc(widget.eventId);
